 ...
 *            the generic type
 */
public class BioJavaSequenceAdapter<T extends Compound> implements
		ProxySequenceReader<T> {
	...
	public BioJavaSequenceAdapter(final ByteSeq<? extends ByteAlphabet> sequence) {
		...
		this.compoundFactory = BioJavaFactoryFactory
				.getCompoundFactory(sequence.getAlphabet());
	}
	...
	public void setCompoundSet(final CompoundSet<T> compoundSet) {
		...
		compoundFactory = BioJavaFactoryFactory.getCompoundFactory(sequence
				.getAlphabet());
	}
