...
public class TwoBitByteCodec extends AbstractByteCodec {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(TwoBitByteCodec.class.getName());
	...
	public byte[] decode(final ByteAlphabet alphabet, final byte[] encodedData,
		...
		while (decodedPos < endPos) {
			...
			decodedData[decodedPos++] = alphabet
					.getByteSymbolForOrdinal((encodedByte & MASK_1) >> SHIFT_1);
			decodedData[decodedPos++] = alphabet
					.getByteSymbolForOrdinal((encodedByte & MASK_2) >> SHIFT_2);
			decodedData[decodedPos++] = alphabet
					.getByteSymbolForOrdinal(encodedByte & MASK_3);
		}
	...
	public byte decode(final ByteAlphabet alphabet, final byte[] encodedData,
		...
		return alphabet
				.getByteSymbolForOrdinal((encodedData[bytePos] & MASK[charPos]) >> SHIFT[charPos]);
	}
	...
	public byte[] encode(final ByteAlphabet alphabet,
		...
		while (decodedPos < endPos) {
			encodedData[encodedPos++] = (byte) ((alphabet
					...
					| (alphabet.getOrdinalForSymbol(decodedData[decodedPos++]) << SHIFT_2) | (alphabet
	...
	public void encode(final ByteAlphabet alphabet, final byte[] encodedData,
		...
		final int bytePos = pos / SYMBOLS_PER_BYTE;
		final int charPos = pos % SYMBOLS_PER_BYTE;
		encodedData[bytePos] = (byte) (encodedData[bytePos] & ~MASK[charPos] | (alphabet
				.getOrdinalForSymbol(symbol) << SHIFT[charPos]));
	}
