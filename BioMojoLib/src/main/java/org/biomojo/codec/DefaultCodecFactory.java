...
public class DefaultCodecFactory extends AbstractIdBasedFactory<Codec<?, ?>> {
	public DefaultCodecFactory() {
		super(new Codec<?, ?>[0]);
		registerProvider(CodecId.NULL_BYTE_CODEC,
				new ConfiguredObjectProvider<Codec<?, ?>>() {
					@Override
					public Codec<?, ?> getObject(
							final Config<Codec<?, ?>> config)
							throws FactoryException {
						return new NullByteCodec((int) config.values()[0]);
					}
				}, true);
		registerProvider(CodecId.TWO_BIT_BYTE_CODEC,
				new ConfiguredObjectProvider<Codec<?, ?>>() {
					@Override
					public Codec<?, ?> getObject(
							final Config<Codec<?, ?>> config)
		...
		registerProvider(CodecId.THREE_BIT_BYTE_CODEC,
				new ConfiguredObjectProvider<Codec<?, ?>>() {
					@Override
					public Codec<?, ?> getObject(
							final Config<Codec<?, ?>> config)
		...
		registerProvider(CodecId.FOUR_BIT_BYTE_CODEC,
				new ConfiguredObjectProvider<Codec<?, ?>>() {
					@Override
					public Codec<?, ?> getObject(
							final Config<Codec<?, ?>> config)
		...
		registerProvider(CodecId.ZLIB_BYTE_CODEC,
				new ConfiguredObjectProvider<Codec<?, ?>>() {
					@Override
					public Codec<?, ?> getObject(
							final Config<Codec<?, ?>> config)
