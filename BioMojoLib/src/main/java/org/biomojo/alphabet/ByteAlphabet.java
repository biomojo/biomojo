...
public interface ByteAlphabet extends Alphabet<Byte> {
	...
	default void validate(final byte[] symbols, final int start, final int end)
	 ...
	 * @return the canonical
	 */
	default byte[] getCanonical(final byte[] values, final int start,
			final int end) {
	 ...
	 *            the end
	 */
	default void makeCanonical(final byte[] symbols, final int start,
			final int end) {
		for (int i = start; i < end; ++i) {
