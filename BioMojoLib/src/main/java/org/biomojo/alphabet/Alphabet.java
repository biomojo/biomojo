...
public interface Alphabet<T> extends IntegerIdentified {
	 ...
	 * @return true all the symbols is a member of this alphabet.
	 */
	public default boolean isValid(final T[] symbols, final int start,
			final int end) {
	...
	public default void validate(final T[] symbols)
	...
	public default void validate(final T[] symbols, final int start,
			final int end) throws InvalidSymbolException {
	 ...
	 *            the end
	 */
	public default void makeCanonical(final T[] symbols, final int start,
			final int end) {
		for (int i = start; i < end; ++i) {
