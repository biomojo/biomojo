 ...
 *
 */
public abstract class AbstractByteAlphabet extends AbstractAlphabet<Byte>
		implements ByteAlphabet {
	private static final Logger logger = LoggerFactory
			.getLogger(AbstractByteAlphabet.class.getName());
	...
	protected void addSymbol(final byte symbol) {
		if (Character.isLetter(symbol)) {
			addSymbol((byte) Character.toUpperCase(symbol),
					(byte) Character.toUpperCase(symbol));
			if (!isCanonical()) {
				addSymbol((byte) Character.toLowerCase(symbol),
						(byte) Character.toUpperCase(symbol));
			}
		} else {
	...
	protected void addSymbol(final byte symbol, final byte canonicalSymbol) {
		logger.debug("addSymbol: symbol = {}, canonicalSymbol = {}",
				(char) symbol, (char) canonicalSymbol);
	 ...
	 */
	@Override
	public final boolean isValid(final byte[] symbols, final int start,
			final int end) {
	...
	@Override
	public void validate(final byte[] symbols, final int start, final int end)
