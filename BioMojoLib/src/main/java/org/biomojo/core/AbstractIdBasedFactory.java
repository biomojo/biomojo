 ...
 *
 */
public class AbstractIdBasedFactory<T extends IntegerIdentified> implements
		IdBasedFactory<T> {
	private static final Logger logger = LoggerFactory
			.getLogger(AbstractIdBasedFactory.class.getName());
	...
	public void registerProvider(final int objectId,
			final ConfiguredObjectProvider<T> provider, final boolean singleton) {
		...
		if (oldProvider != null) {
			throw new UncheckedException(
					"Duplicate object id registered in factory, objectId = "
							+ objectId + ", current class = "
							+ oldProvider.getObject().getClass().getName()
							+ ", new class = "
			...
			if (objectCache[object.getId()] != null) {
				throw new UncheckedException(
						"Duplicate object id registered in factory, objectId = "
								+ object.getId()
								+ ", current class = "
								+ objectCache[object.getId()].getClass()
										.getName() + ", new class = "
	...
	public <Z extends T> Z getInstance(final int objectId, final Class<Z> type) {
		...
		if (instance == null) {
			final ConfiguredObjectProvider<T> provider = providers
					.get(objectId);
			if (provider != null) {
				logger.debug("Constructing new object for objectId {}",
						objectId);
				instance = (Z) provider
						.getObject(new IdBasedConfig<T>(objectId));
