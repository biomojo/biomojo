...
public enum LinuxTimeCommandField implements Field {
	COMMAND_LINE("C", STRING_TYPE), AVG_UNSHARED_DATA("D", MEMORY_TYPE), WALL_CLOCK_HMS(
			"E", STRING_TYPE), MAJOR_FAULTS("F"), NUM_FS_INPUTS("I"), AVG_TOTAL_MEM(
			"K", MEMORY_TYPE), MAX_RSS("M", MEMORY_TYPE), NUM_FS_OUTPUTS("O"), PERCENT_CPU(
			"P", STRING_TYPE), MINOR_FAULTS("R"), SYSTEM_TIME("S", TIME_TYPE), USER_TIME(
			"U", TIME_TYPE), SWAPPED_OUT("W"), AVG_TEXT_MEM("X", MEMORY_TYPE), INV_CONTEXT_SWITCHES(
			"c"), WALL_CLOCK_TIME("e", TIME_TYPE), SIGNALS_DELIVERED("k"), AVG_STACK_MEM(
			"p", MEMORY_TYPE), SOCKET_MSG_REC("r"), SOCKET_MSG_SENT("s"), AVG_RSS(
			"t", MEMORY_TYPE), VOL_CONTEXT_SWITCHES("w");
